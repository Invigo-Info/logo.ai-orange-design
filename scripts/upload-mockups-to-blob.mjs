...
const BASE = "public/logo-mockups";
const urlMap = {};
...
for (const folder of folders) {
  ...
  for (const file of files) {
    ...
    const blobPath = `logo-mockups/${folder}/${file}`;
    ...
    totalUploaded++;
    console.log(`[${totalUploaded}] ${blobPath}`);
  }
}

await writeFile("src/data/mockupBlobUrls.json", JSON.stringify(urlMap, null, 2));
console.log("\nAll uploaded! URL map saved to src/data/mockupBlobUrls.json");
