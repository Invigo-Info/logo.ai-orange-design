...
const POSTS = [
  ...
  },
  {
    ...
    excerpt:
      "When to pick a wordmark, an icon, or both — and why it matters.",
    img: "https://images.unsplash.com/photo-1568126872850-f1a5d52f12ab?w=720&q=70&auto=format&fit=crop",
  },
];
...
export default function Blog() {
  return (
    ...
    >
      ...
      <div className="max-w-[1240px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {POSTS.map((post, i) => (
          <ScrollReveal key={post.title} delay={(i % 3) as 0 | 1 | 2 | 3}>
            <article className="group h-full rounded-2xl border border-cream-10 bg-b1 overflow-hidden flex flex-col hover:border-cream-18 transition-all duration-300 hover:-translate-y-1 cursor-pointer">
              {/* Cover image */}
